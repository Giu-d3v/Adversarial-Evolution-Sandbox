...
**顶部 HUD 左半边**：
- **策略占比**：用 `classify_agent` 数每种标签有多少个体，**全部 5 个标签按固定顺序**显示（0 计数的也展示），格式：`夺利 23 互助合作 18 好善疾恶 21 修身 18 未分化 52`
- **阶段标签**：见 §2.3
- **善者率** = `(互助合作 + 好善疾恶) / pop`，即 `cooperation ≥ 0.6` 的个体占比

**顶部 HUD 右半边 · 策略分布条形图**（5 行 = 5 个策略类别）：
...
- **标签** = 5 个策略名（中文）
- **条颜色** = 该策略的 hue（用 `_hue_for_label(label)` 反查）—— **跟世界里的圆点同色系**，玩家一眼能对上
- **条长度** = 该类别 count / max(all counts) × 可用宽度（条按最大类归一化）
- **数字** = 该策略的实际 count
- **0 计数**也画出灰色 track，不隐藏类别（玩家能直观看到"这个类别当前没人"）

**善者率 sparkline**：最近 220 tick 的折线，0.5 处一条参考线。
...
| **善者率 sparkline** | `World.good_hist[]` 220 tick 环形缓冲 |
...
| **善者率波动** | sparkline 直接画出 | seed=99：46% → 40%（稳定下降）；seed=42：50% → 49%（震荡）|
